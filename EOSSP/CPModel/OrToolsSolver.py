        ...
        def resolve(self,constellation,mailbox,afficher=True):
            ...
            if tlim>0:
                with open("test_output",'w') as file:
                    path = config.glob.docplexPath
                    if config.getOptValue("threads")is not None:
                        if path!="":
                            sol = self.model.solve(TimeLimit=tlim,execfile=path,Workers=config.getOptValue("threads"),log_output=file)
                        else:
                            sol = self.model.solve(TimeLimit=tlim,Workers=config.getOptValue("threads"),log_output=file)
                    else:
                        if config.getOptValue("time")<np.Inf:
                            if path!="":
                                sol = self.model.solve(TimeLimit=tlim,execfile=path,log_output=file)
                            else:
                                sol = self.model.solve(TimeLimit=tlim,log_output=file)
                        else:
                            if path!="":
                                sol = self.model.solve(execfile=path,log_output=file)
                            else:
                                sol = self.model.solve(log_output=file)
