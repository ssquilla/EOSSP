...
class TimeIndependentTransitionModel:
    ...
    def usesTimeDependentSolver(self):
        return False
    ...
    def getTransititionDurationUpperBound(self):
        return self.tau_max
    ...
    def getTransitionDuration(self,satellite,p1,p2,digits):
        return self.computeTransition(satellite,p1,p2,digits)
...
class ExtractedTimeIndependentModel:
...
class AveragePrecomputedModel(ExtractedTimeIndependentModel):
    ...
    def getTransitionDuration(self, satellite, a1, a2, digits):
        if a1==a2:
            return 0
        index = self.getAbsoluteIndexOfPairApprox(a1,a2)
        return round(self.tabular[index],digits)
    
class MinPrecomputedModel(ExtractedTimeIndependentModel):
    def __init__(self,modeleTimeDep):
...
class MaxPrecomputedModel(ExtractedTimeIndependentModel):
    def __init__(self,modeleTimeDep):
...
class TimeDependentTransitionModel:
    ...
    def createTransitionModelFromFiles(self,foldername,constellation,composantes,precompute_approx=None):
        ...
        self.initComponentsData(composantes) 
                    ...
                    for k,(a2,debut) in enumerate(activites):
                        ...
                        for i in range(nValues): # verifier que les activites sont dans le mapping (car certaines obs peuvent etre ignorees. systematiques par exemple)
                            if a1 != -1 and a2 != -1 and a1 in self.mapping_cca and a2 in self.mapping_cca:
                                ...
                                valeur = closestRounding((value-durees[a1]),digits)
                        ...
                        if a1 != -1 and a2 != -1 and a1!=a2 and a1 in self.mapping_cca and a2 in self.mapping_cca:
                            if np.isinf(min_t):
                                min_t = 0
                            mean_t = closestRounding(mean_t / nValues,3)
                            index = self.getAbsoluteIndexOfPairApprox(a1,a2)
    ...
    def initComponentsData(self,composantes):
    ...
    def getTransitionAverage(self,a1,a2):
        index = self.getAbsoluteIndexOfPairApprox(a1,a2)
    ...
    def getControlPoints(self,a1,a2,time=True):
    ...
    def findClosestDatesPair(self,a1,a2,t):
    ...
    def getTransitionDuration(self,a1,a2,t):
        if a1==a2:
            return 0
        start_index = self.getAbsoluteIndexOfPairTimeDep(a1,a2)
        if start_index==-1:
        ...
        res = self.findClosestDatesPair(a1, a2, t)
        if res is None:
            return 0
    ...
    def getTransitionDurationUpperBoundForActivityPair(self,a1,a2):
        if a1==a2:
            return 0
...
global SlowModel
SlowModel = TimeIndependentTransitionModel(0.8,0.8,90,"slow")

global FastModel
FastModel = TimeIndependentTransitionModel(10,15,90,"fast")

global MeanModel
MeanModel = TimeIndependentTransitionModel(1,1,90,"mean")